...
MongoStore._mongoUuid = function(uuid) {
  var bytes = Buffer.from(uuid.replace(/-/g, ''), 'hex');
  return new mongodb.Binary(new Uint8Array(bytes), mongodb.Binary.SUBTYPE_UUID);
};
...
MongoStore.prototype.initialise = function(resourceConfig) {
  ...
  mongodb.MongoClient.connect(self._config.url).then(function(client) {
    self._db = client.db();
 ...
 */
MongoStore.prototype.populate = function(callback, drop) {
  var self = this;
  if (!self._db) return;
  
  if(drop === true){
  self._db.dropDatabase().then(function() {
    self.createExamples(callback, self);
  }).catch(function(err) {
...
MongoStore.prototype.createExamples = function(callback, self) {
    const uuidV4 = require('uuid/v4');
    console.log(self.resourceConfig);
    async.each(self.resourceConfig.examples, function(document, cb) {
      ...
      if(!document.type) {
          document.type = self.resourceConfig.type;
      }
...
MongoStore.prototype.search = function(request, callback) {
  ...
  Promise.all([
    (function() {
      var cursor = collection.find(criteria, { projection: { _id: 0 } });
      cursor = self._applySort(request, cursor);
      cursor = self._applyPagination(request, cursor);
...
MongoStore.prototype.find = function(request, callback) {
  ...
  console.log("findOne", JSON.stringify({ _id: documentId }));
  collection.findOne({ _id: documentId }, { projection: { _id: 0 } }).then(function(result) {
    if (!result) {
...
MongoStore.prototype.create = function(request, newResource, callback) {
  ...
  collection.insertOne(document).then(function() {
    return collection.findOne(document, { projection: { _id: 0 } });
...
MongoStore.prototype.delete = function(request, callback) {
  var collection = this._db.collection(request.params.type);
  var documentId = MongoStore._mongoUuid(request.params.id);
  collection.deleteOne({ _id: documentId }).then(function(result) {
